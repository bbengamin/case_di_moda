...
class ControllerAccountRegister extends Controller {
    ...
    public function add() {
        ...
        $this->load->model('account/customer');
        $this->load->model('account/fast_customer');
        $json = array();
        if ((utf8_strlen($this->request->post['email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['email'])) {
            $json['email'] = "Введен некорректный адрес электронной почты";
        }
        if($this->model_account_customer->getTotalCustomersByEmail($this->request->post['email'])){
            $json['email'] = "Такой email уже зарегистрирован";
        }
        /*if(utf8_strlen($this->request->post['password']) < 3){
            $json['password'] = "Слабый пароль";
        }

        if($this->request->post['password'] != $this->request->post['repeat-password']){
            $json['password'] = "Пароли не совпадают";
        }
*/
        if(!$json){
            $this->load->model('account/customer');
            $this->load->model('account/fast_customer');
            $pass = $this->model_account_fast_customer->addCustomer2($this->request->post);
